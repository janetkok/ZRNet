...
class ZRNet_ab(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=12, enc_blk_nums=[2, 2, 4, 8],
        ...
        self.n_s1 = 3
        self.n_s2 = 11
        self.total_ab_g = self.n_s1 + self.n_s2 
    ...
    def forward(self, inp):
        ...
        #separate MLPs
        for i,zi in zip(range(self.n_s1),[1,9,21]):
            s1_unit[i,:,0,:] =  getattr(self.zern_mlp, f'zern_mlp_{zi}')(zernike1d)
        for i,zi in zip(range(self.n_s2),[[0,2],[3,6],[4,5],[7,11],[8,10],[12,17],[13,16],[14,15],[18,24],[19,23],[20,22]]): 
            for j in range(2):
        ...
        for b in range(batch): 
            ...
            # information exchange between aberration groups
            r_data = torch.cat([s1_unit[:,b,:,:], s2_r[:,b,:,:]],dim=0).squeeze() 
            r = self.relation_zern_r2r(r_data,adj_r2r)

            s1_unit[:,b,0,:] = r[:self.n_s1,:]
            s2_r[:,b,0,:] = r[self.n_s1:,:]
