...
class ZRNet_mlp(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=12, enc_blk_nums=[2, 2, 4, 8],
        ...
        for i in range(self.num_zernike):
            self.zern_mlp.add_module('zern_mlp_{}'.format(i), Mlp(chan,1))
    ...
    def forward(self, inp):
        ...
        # separate MLPs for Zernike coefficient prediction
        for i in range(self.num_zernike):
            zernike[:,i]=getattr(self.zern_mlp, f'zern_mlp_{i}')(zernike1d).squeeze()
