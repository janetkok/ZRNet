...
class ZRNet_nogrp(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=12, enc_blk_nums=[2, 2, 4, 8],
        ...
        self.relation_zern_n2n = GraphAttentionLayer(self.zern_dim, self.zern_dim, 0.2, 0.2, concat=True)
    ...
    def forward(self, inp):
        ...
        unit = torch.zeros(batch,self.num_zernike,self.zern_dim, device='cuda') 
        adj_n2n = torch.ones(self.num_zernike,self.num_zernike, device='cuda')
        zernike = torch.zeros(batch,self.num_zernike, device='cuda')

        for i in range(self.num_zernike):
            unit[:,i,:]=getattr(self.zern_mlp, f'zern_mlp_{i}')(zernike1d)

        # information exchange between Zernike nodes
        for b in range(batch):
            unit_data = unit[b,:,:].clone()
            unit[b,:,:] = self.relation_zern_n2n(unit_data,adj_n2n)

        for i in range(self.num_zernike):
            zernike[:,i]=getattr(self.zern_mlp_pred, f'zern_mlp_pred{i}')(unit[:,i,:]).squeeze()
